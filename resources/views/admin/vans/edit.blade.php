...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.van.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vans.update", [$van->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.van.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $van->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $van->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $van->price) }}" step="0.01" required>
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_number_of_vans') ? 'is-invalid' : '' }}" type="number" name="total_number_of_vans" id="total_number_of_vans" value="{{ old('total_number_of_vans', $van->total_number_of_vans) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_enabled') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_enabled" id="is_enabled" value="1" {{ $van->is_enabled || old('is_enabled', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facilities') ? 'is-invalid' : '' }}" name="facilities[]" id="facilities" multiple required>
                    @foreach($facilities as $id => $facility)
                        <option value="{{ $id }}" {{ (in_array($id, old('facilities', [])) || $van->facilities->contains($id)) ? 'selected' : '' }}>{{ $facility }}</option>
                    @endforeach
                </select>
