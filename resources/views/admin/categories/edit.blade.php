    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_seats') ? 'is-invalid' : '' }}" type="number" name="total_seats" id="total_seats" value="{{ old('total_seats', $category->total_seats) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facilities') ? 'is-invalid' : '' }}" name="facilities[]" id="facilities" multiple>
                    @foreach($facilities as $id => $facility)
                        <option value="{{ $id }}" {{ (in_array($id, old('facilities', [])) || $category->facilities->contains($id)) ? 'selected' : '' }}>{{ $facility }}</option>
                    @endforeach
                </select>
