...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.booking-statuses.update', [$bookingStatus->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('booking') ? 'is-invalid' : '' }}" name="booking_id"
                        ...
                        @foreach ($bookings as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('booking_id') ? old('booking_id') : $bookingStatus->booking->id ?? '') == $id ? 'selected' : '' }}>
                                Booking Id:{{ $id }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach (App\Models\BookingStatus::STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('status', $bookingStatus->status) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message"
                        id="message">{!! old('message', $bookingStatus->message) !!}</textarea>
