    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('van') ? 'is-invalid' : '' }}" name="van_id" id="van_id" required>
                    @foreach($vans as $id => $entry)
                        <option value="{{ $id }}" {{ old('van_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('van'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('drop_location') ? 'is-invalid' : '' }}" type="text" name="drop_location" id="drop_location" value="{{ old('drop_location', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('pickup_date') ? 'is-invalid' : '' }}" type="text" name="pickup_date" id="pickup_date" value="{{ old('pickup_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('drop_date') ? 'is-invalid' : '' }}" type="text" name="drop_date" id="drop_date" value="{{ old('drop_date') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_booking_amount') ? 'is-invalid' : '' }}" type="number" name="total_booking_amount" id="total_booking_amount" value="{{ old('total_booking_amount', '') }}" step="0.01" required>
